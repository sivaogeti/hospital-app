...
// Pages (ensure these files exist and accept `user` map)
...
class _HomePageState extends State<HomePage> {
  ...
  // ---------- helpers to push pages passing user map ----------
  void _openPatients() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => PatientsPage(user: widget.user)));
  }

  void _openRecordVitals() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => RecordVitalsPage(user: widget.user)));
  }
  ...
  void _openSugarTest() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => SugarTestPage(user: widget.user, username: widget.user?['username']),
      ),
    );
  ...
  void _openProfiles() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => ProfilesPage(user: widget.user)));
  }

  void _openReports() {
    Navigator.push(context, MaterialPageRoute(builder: (_) => ReportsPage(user: widget.user)));
  }
                ...
                child: Padding(
                  ...
                  child: LayoutBuilder(builder: (ctx, constraints) {
                    ...
                    return GridView.builder(
                      ...
                      itemBuilder: (c, i) {
                        if (i == 0) return _dashboardCard('Patients', '4', Icons.people_alt, _openPatients);
                        if (i == 1) return _dashboardCard('Record Vitals', '2924', Icons.monitor_heart, _openRecordVitals);
                        if (i == 2) return _dashboardCard('Sugar Blood Test', '2924', Icons.science, _openSugarTest);
                        if (i == 3) return _dashboardCard('Inventory / Stock', '0', Icons.inventory_2_outlined, _openInventory);
                        if (i == 4) return _dashboardCard('Health Profiles', '', Icons.bar_chart, _openProfiles);
                        return _dashboardCard('Reports', '', Icons.article_outlined, _openReports);
                      },
                    );
                  }),
                ),
